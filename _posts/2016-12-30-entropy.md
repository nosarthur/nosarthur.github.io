...
$$ S = \lim_{N\rightarrow\infty}\frac{C(N,N_H)}{N} $$
...
In physics, it is postulated that the probability of a state is proportional to $$ \exp(-\frac{E}{K_B T})$$ where $$E$$ is the energy of the state.
...
in contrast to the directed acyclic graph structure or even tree structure in math.
...
On the other hand, the information theory approach is like pure math.
It doesn't depend on many concepts and is relatively concise.
You can probably learn it much faster than statistical physics.
